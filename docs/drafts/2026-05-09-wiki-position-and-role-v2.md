...
> **Companion docs:** `addition-items-v1.md`, `addition-items-v2.md`, `issue-v2.md`
...
**Implementation:** Build from existing components (`registry.json`, `events.jsonl`, `wiki_lint`, `wiki_scan_activity`). Trigger in the same pipeline as `rebuildAllGeneratedArtifacts()`. No new tools needed.
...
```
Wiki/discussions/
├── route.md              ← simple markdown with frontmatter
...
|----------|-------|-----|--------|
| **1** | Wiki digest (#2) | Biggest lever. Unblocks "Wiki first" posture. Builds from existing code. | Small |
| **2** | Discussion system MVP (#3) | Files only, no new tools. Enables session continuity. | Small |
| **3** | Staleness detection (#5) | Small change, big impact. Makes Wiki trustworthy. | Small |
| **4** | Draft/ position (#7) | Clean cut. Simplifies zone map. Phased with deprecation warnings. | Medium |
| **5** | Link resolution (#8) | Small utility. Enables agent to read PARA via Wiki links. | Small |
| **6** | Wiki→PARA principle (#1) | Update skills + diagrams. Dotted arrows. | Small |
| **7** | Lifecycle triggers (#6) | Formalize grace periods in existing lint. | Small |
| **8** | Session start (#9) | Update skills once #2 and #3 exist. | Small |
| **9** | Drafts vs plans (#10) | Skill instructions + user convention. | Small |
| **10** | wiki_sync bootstrap (#11) | Change description, add `meta/sync-state.json`. | Small |
| **11** | Route scalability (#4) | Year 2 problem. Don't solve now. | — |
...
| # | Change | Source |
|---|--------|--------|
| 1 | Added `discussions/` folder to vault structure | `addition-items-v1.md` #1 |
| 2 | Added zone ownership table | `addition-items-v1.md` #7 |
| 3 | Resolved Wiki→PARA contradiction — "human decides, not automatic" | `addition-items-v2.md` #1 |
| 4 | "Daily digest" → Wiki digest (`meta/wiki-digest.md`) | `addition-items-v1.md` #2, #3 |
| 5 | Added session start sequence: `route.md → wiki-digest.md → LIST.md` | `addition-items-v1.md` #8 |
| 6 | Added full topic lifecycle states with reactivation rule | `addition-items-v1.md` #6 |
| 7 | Added `wiki_sync` bootstrap semantics | `addition-items-v1.md` #5, `addition-items-v2.md` #11 |
| 8 | Added briefing file format | `addition-items-v1.md` #4 |
| 9 | Added Draft/ migration plan | `addition-items-v2.md` #7 |
| 10 | Added staleness detection (`last_synced` + lint) | `addition-items-v2.md` #5 |
| 11 | Added lifecycle grace periods (lint suggests, human decides) | `addition-items-v2.md` #6 |
| 12 | Added link resolution utility | `addition-items-v2.md` #8 |
| 13 | Added drafts vs plans boundary | `addition-items-v2.md` #10 |
| 14 | Added implementation priority table | `addition-items-v2.md` summary |
